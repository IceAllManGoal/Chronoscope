...
public sealed record ProcessCollectorSettings
{
    ...
    /// <summary>
    /// Собирать путь к исполняемому файлу. По умолчанию <b>выключено</b>.
    ///
    /// Ключа нет в примере §36, но §37 относит путь к уровню Standard, а Minimal
    /// ограничивает набор полями «name, pid, parent relation, timestamps». Без
    /// отдельного флага уровень Minimal был бы недостижим, а путь содержит имя
    /// пользователя в профиле (<c>C:\Users\...</c>) и выдаёт установленное
    /// программное обеспечение.
    /// </summary>
    public bool CapturePath { get; init; }
