...
**0.0.3 — выпущена.** 0.0.1 доказала, что вертикальный срез работает целиком, 0.0.2 сделала поток событий видимым, а 0.0.3 научила воспринимать **один экземпляр процесса как сущность во времени** — §77 «Identify». Это по-прежнему не MVP продукта, а работающий вертикальный срез:
...
ProcessNormalizer
   ↓
Event  ──►  process_instance_id  (host + boot + PID + время старта)
   ↓                                ↓
GET /api/v1/events          GET /api/v1/processes/{id}
   ↓                                ↓
CLI `chronoscope …`  и  страница http://127.0.0.1:7342/ui/
```

Событие отвечает на вопрос «что произошло», экземпляр процесса — «что это за процесс, когда жил и кто его запустил». Первое появилось в 0.0.2, второе — в 0.0.3.
...
В 0.0.3 к этому добавился сценарий §77.1 — на изолированной конфигурации, с настоящими Agent и Core: `powershell` запустил `notepad`, `notepad` завершился, и жизнь экземпляра (запуск, завершение, длительность 18.8 с) восстановлена через API, CLI и страницу. Наблюдаемая цепочка `powershell → cmd.exe → ping.exe` разрешилась на всех трёх уровнях. У приложения из Microsoft Store родитель процесса приложения не наблюдается — это измерено и записано в [`agent/README.md`](agent/README.md), и связь в таком случае честно помечается как неустановленная, а не выдумывается.
...
## Чего Chronoscope не утверждает

Одно различие стоит знать до того, как смотреть историю: **время известно** и **событие наблюдалось** — разные вещи.

Время старта экземпляра процесса может быть известно и без наблюдавшегося события запуска: событие завершения несёт `process_started_at`, потому что иначе его нельзя связать с запуском. Поэтому пустое завершение означает «**завершения не наблюдалось**» и ничего больше — это **не** значит, что процесс работает. Коллектор опрашивает систему раз в секунду и может пропустить событие выхода, поэтому Chronoscope не выводит состояние процесса из истории: он показывает то, что видел, и отдельно помечает, что именно видел.

Одно и то же правило действует в API, в CLI и на странице: строка «Наблюдался выход — нет» рядом с известным временем запуска — не противоречие, а честный ответ.
...
| `GET /api/v1/processes/{process_instance_id}` | Экземпляр процесса: идентичность, жизнь, родитель |
...
uv run chronoscope process proc_01K5...            # экземпляр процесса: жизнь и родитель
uv run chronoscope doctor                          # диагностика
```

`chronoscope process` показывает то же, что `GET /api/v1/processes/{id}`: имя, PID, boot, времена запуска и завершения, длительность, родителя и — отдельными строками — наблюдались ли события запуска и выхода. События этого экземпляра остаются делом `events --subject-id`.
