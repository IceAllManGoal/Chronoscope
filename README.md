...
| **Chronoscope Core**: приём, нормализация, хранение, API | **готово**, 339 тестов |
...
| Экземпляр процесса — 0.0.3 «Identify» (§77.1) | в работе: `GET /api/v1/processes/{id}` собирает экземпляр из его событий и честно различает «время известно» и «событие наблюдалось»; переходы из UI, команда CLI и остальные пункты §77.1 — ещё нет |
...
```powershell
cd core
uv run pytest        # 339 тестов: домен, хранилище, нормализатор, контракт, правила слоёв, приём, индексы, экземпляр процесса, CLI, страница, скрипты, интеграция
