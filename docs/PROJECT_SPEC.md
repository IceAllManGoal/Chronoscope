...
11. **Сделано измерение нагрузки, а не догадка о ней.** Не менее часа обычной работы с обоими источниками, и **первый прогон идёт без семантической фильтрации**: только `capture_message = false`, то есть приватность по умолчанию, и никаких фильтров по уровню, провайдеру или `event_id`. Фильтр, добавленный до измерения, означал бы измерение собственной гипотезы вместо Windows Event Log.

    Записывается: событий в час по каждому источнику; распределение по `provider` и по `event_id` — какие провайдеры шумят и какие идентификаторы доминируют; сырых событий в час; рост файла базы; CPU и память Agent и Core; отзывчивость API и страницы.

    Высокая плотность сама по себе проблемой не является: 10 000 событий в час при нулевой нагрузке и стабильной памяти — это один разговор, те же 10 000 событий с заметной нагрузкой — другой. Решение о фильтре принимается **отдельно и по измеренным данным**, а не по ощущению «слишком много строк»: если большую часть потока даёт один провайдер или один `event_id`, это станет видно из распределения.

    *Чем подтверждается:* протокол измерения с числами; первый час сохраняется как baseline версии — в `CHANGELOG.md` и в `docs/DEVELOPMENT.md` (раздел «Измерения»), чтобы следующая версия сравнивалась с числом, а не с воспоминанием. Если объём окажется неудобным, это так и написано, а не сглажено. Искусственные лимиты по итогам измерения сразу не вводятся.
