...
8. **Реальный end-to-end сценарий восстанавливает цепочку.** На живой машине `powershell` запускает `notepad`, `notepad` завершается, после чего цепочка восстанавливается через UI и через API. Сценарий прогоняется вручную, на настоящих компонентах и изолированной конфигурации, и в записи о версии остаётся, чем он подтверждён, — как это сделано для 0.0.2.
   *Чем подтверждается:* прогон перед выпуском; результат — в `CHANGELOG.md` и в описании релиза.
