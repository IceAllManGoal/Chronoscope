...
|---|---|---|---|
| **Chronoscope Agent** | Наблюдает за Windows и доставляет сырые события в Core | C# / .NET | 0.0.1, планируется |
| **Chronoscope Core** | Принимает, нормализует, хранит и отдаёт события | Python, FastAPI, SQLAlchemy, Alembic, SQLite | 0.0.1, планируется |
